 ...
 */
class CustomModelTabsModel extends MY_Model
{
    public $table = 'custom_model_tabs';
    public $primaryKey = 'custom_model_tab_id';

    public $hasMany = [
        'custom_model_fields' => ['custom_model_tab_id', 'Admin/CustomModelFieldsModel', 'CustomModelFieldsModel'],
    ];
    ...
    {
        $this->load->model('Admin/CustomModelFieldsModel');
        foreach ($collection as $key => &$value) {
            if (isset($value->custom_model_tab_id) && $value->custom_model_tab_id) {
                $custom_model_fields = new CustomModelFieldsModel();
