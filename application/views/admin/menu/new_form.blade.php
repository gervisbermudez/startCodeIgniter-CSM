	...
	<div class="container form" id="root">
		...
		<div class="row">
			...
			<div id="form" class="col s12" v-bind:class="{'m10': user_id}" v-cloak v-show="!loader">
				...
				<br />
				<div>
					<a href="#!" class="btn" v-on:click="addItem()">Agregar Item <i class="material-icons right">add_box</i></a>
				</div>
				<br />
				<ul class="collapsible expandable sorteable" >
					<li v-for="(item, index) in menu_items" :key="index" :data-id="index" :data-name="item.item_name">
						...
						<div class="input-field">
							...
							<input type="text" v-model="item.item_link" :id="'item_link' + index" required="required"
								value="">
						</div>
						<div class="input-field">
							<label class="active" for="'item_target' + index">Target:</label>
							<input type="text" v-model="item.item_target" :id="'item_target' + index" required="required"
								value="">
						</div>
						<div class="input-field">
							<label class="active" for="'item_title' + index">Title:</label>
							<input type="text" v-model="item.item_title" :id="'item_title' + index" required="required"
								value="">
						</div>
						</div>
					</li>
