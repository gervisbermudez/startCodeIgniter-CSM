		...
		<div class="row">
			<form action="<?php echo base_url($action);?>" method="post" id="form" class="col s12 m10 l10">
				...
				<div class="input-field">
					...
					<input type="text" id="nombre" name="nombre" required="required" value="<?php echo element('nombre', $video, ''); ?>">
				</div>
				...
				<div class="input-field">
					...
					<input type="text" id="duracion" name="duration" required="required" value="<?php echo element('duration', $video, ''); ?>">
				</div>
				<div class="input-field">
					<label for="youtubeid" >URL Youtube:</label>
					<input type="text" id="youtubeid" name="youtubeid" required="required" value="<?php echo element('youtubeid', $video, ''); ?>">
				</div>
				<div class="input-field">
					<label for="paypal" >Informacion de pago:</label>
					<input type="text" id="paypal" name="paypal" required="required" value="<?php echo element('payinfo', $video, '');?>">
				</div>
				<span  id="preview" class="header grey-text text-darken-2 scrollspy">Preview <i class="material-icons left">perm_media</i></span>
				<div class="input-field" >
					<input  type="text" id="imagen" class="modal-trigger" name="preview" placeholder="Imagen principal" data-expected="inactive" data-target="modal1"  value="<?php echo element('preview', $video, '');?>">
				</div>
				<br>
				<div class="input-field">
					<select name="categorias[]" multiple>
						...
						<?php if ($categorias): ?>					
							<?php foreach ($categorias as $key => $categoria): ?>
								<option value="<?php echo $categoria['id'] ?>" <?php if (array_key_exists($categoria['id'], $videocategoria)): ?>
									selected
								<?php endif ?> ><?php echo $categoria['nombre'] ?></option>
							<?php endforeach ?>
						<?php endif ?>
					</select>
