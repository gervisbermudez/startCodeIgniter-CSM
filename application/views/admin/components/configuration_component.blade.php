			...
			<div class="collapsible-body" :class="{ 'collapsible-body-hidden': !show_body}" v-show="show_body">
				...
				<div v-if="handle_value_as == 'select' && configuration.config_data.perm_values">
					<select v-model="configuration.config_value" v-on:change="saveConfig();">
						<option value="" disabled selected><?php echo lang('config_choose_option'); ?></option>
						<option v-for="(value, i) in configuration.config_data.perm_values" v-if="(typeof value == 'string')" :key="i" :value="value">@{{value}}</option>
					</select>
				</div>
				<div class="input-field" v-if="handle_value_as == 'input'">
					<input :id="'config-field-' + configuration.site_config_id" :type="configuration.config_data.input_type || 'text'" class="validate" :class="{ invalid: configuration.validate === false, valid: configuration.validate === true }" v-model="configuration.config_value" v-on:blur="saveConfig();" v-on:focus="focusInput();">
				</div>
