class VueForm {
  ...
  validateField(field) {
    ...
    if (
      this.fields[field].readOnlyValue &&
      this.fields[field].readOnlyValue == this.fields[field].value
    ) {
     ...
     */
    if (
      this.fields[field].validateWith &&
      typeof this.fields[field].validateWith === "function" &&
      !this.fields[field].validateWith(field)
    ) {
  ...
  basicFieldValidation(field) {
     ...
     */
    if (
      this.fields[field].minLength &&
      !this.minLength(this.fields[field].value, this.fields[field].minLength)
    ) {
      this.markFieldAsInvalid(field);
      this.fields[field].errorText =
        "Minlength required: " + this.fields[field].minLength;
      return false;
    }
     ...
     */
    if (
      this.fields[field].maxLength &&
      !this.maxLength(this.fields[field].value, this.fields[field].maxLength)
    ) {
      this.markFieldAsInvalid(field);
      this.fields[field].errorText = this.fields[field].errorText =
        "Maxlength required: " + this.fields[field].maxLength;
      return false;
    }
    ...
    if (this.fields[field].customPattern) {
      ...
      switch (control.type) {
        case "name":
          ...
          this.fields[field].errorText = valid
            ? ""
        ...
        case "email":
          ...
          this.fields[field].errorText = valid
            ? ""
            : "Email don`t have the correct format: example@domain.com";
          break;
        case "cuit":
          valid = this.CUILValidation(control.value);
          this.fields[field].errorText = valid
            ? ""
            : "Cuit don`t have the correct format: ##-########-#";
          break;
        case "date":
        ...
        case "password":
          ...
          this.fields[field].errorText = valid
            ? ""
            : "Include a special character, number and at least one capital and lower letter";
          break;
        default:
          /**
          ...
          if (typeof this.patterns[control.type] != "undefined") {
            ...
            this.fields[field].errorText = valid
              ? ""
              : field.charAt(0).toUpperCase() + field.slice(1) + " is invalid";
          } else {
            valid = false;
          }
          break;
  ...
  getErrorText(field) {
    ...
    return this.fields[field].errorText
      ? this.fields[field].errorText
      : field.charAt(0).toUpperCase() + field.slice(1) + " is invalid";
  }
  /** VALIDATION LIST */
  ...
  numValidation(value, l = 0) {
    if (
      this.patterns.number.test(value) == false ||
      !this.minLength(value, l)
    ) {
      return false;
    }
    return true;
